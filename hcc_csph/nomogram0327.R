...
train_data$mat01_cat <- factor(train_data$mat01, levels = c(0,1))
test_data$mat01_cat  <- factor(test_data$mat01, levels = c(0,1))
train_data$CUPI01_cat <- factor(train_data$CUPI01, levels = c(0,1))
test_data$CUPI01_cat  <- factor(test_data$CUPI01, levels = c(0,1))
...
km_fit_train_mat <- survfit(Surv(OS12, SUV) ~ mat01_cat, data = train_data)
km_fit_test_mat  <- survfit(Surv(OS12, SUV) ~ mat01_cat, data = test_data)
km_fit_train_cupi <- survfit(Surv(OS12, SUV) ~ CUPI01_cat, data = train_data)
km_fit_test_cupi  <- survfit(Surv(OS12, SUV) ~ CUPI01_cat, data = test_data)
####1os
...
km_summary_train_3_mat <- summary(km_fit_train_mat, times = 3)
km_summary_test_3_mat  <- summary(km_fit_test_mat, times = 3)
km_summary_train_5_mat <- summary(km_fit_train_mat, times = 5)
km_summary_test_5_mat  <- summary(km_fit_test_mat, times = 5)

km_summary_train_3_cupi <- summary(km_fit_train_cupi, times = 3)
km_summary_test_3_cupi  <- summary(km_fit_test_cupi, times = 3)
km_summary_train_5_cupi <- summary(km_fit_train_cupi, times = 5)
km_summary_test_5_cupi  <- summary(km_fit_test_cupi, times = 5)
...
mat_groups_train_3 <- names(km_fit_train_mat$strata)
surv_prob_train_3_mat <- km_summary_train_3_mat$surv
mat_groups_test_3 <- names(km_fit_test_mat$strata)
surv_prob_test_3_mat <- km_summary_test_3_mat$surv
mat_groups_train_5 <- names(km_fit_train_mat$strata)
surv_prob_train_5_mat <- km_summary_train_5_mat$surv
mat_groups_test_5 <- names(km_fit_test_mat$strata)
surv_prob_test_5_mat <- km_summary_test_5_mat$surv

cupi_groups_train_3 <- names(km_fit_train_cupi$strata)
surv_prob_train_3_cupi <- km_summary_train_3_cupi$surv
cupi_groups_test_3 <- names(km_fit_test_cupi$strata)
surv_prob_test_3_cupi <- km_summary_test_3_cupi$surv
cupi_groups_train_5 <- names(km_fit_train_cupi$strata)
surv_prob_train_5_cupi <- km_summary_train_5_cupi$surv
cupi_groups_test_5 <- names(km_fit_test_cupi$strata)
surv_prob_test_5_cupi <- km_summary_test_5_cupi$surv
...
mat_levels_train_3 <- gsub("mat01_cat=", "", mat_groups_train_3)
mat_levels_test_3 <- gsub("mat01_cat=", "", mat_groups_test_3)
mat_levels_train_5 <- gsub("mat01_cat=", "", mat_groups_train_5)
mat_levels_test_5 <- gsub("mat01_cat=", "", mat_groups_test_5)

cupi_levels_train_3 <- gsub("CUPI01_cat=", "", cupi_groups_train_3)
cupi_levels_test_3 <- gsub("CUPI01_cat=", "", cupi_groups_test_3)
cupi_levels_train_5 <- gsub("CUPI01_cat=", "", cupi_groups_train_5)
cupi_levels_test_5 <- gsub("CUPI01_cat=", "", cupi_groups_test_5)
...
mat_surv_map_train_3 <- data.frame(
  mat01_cat = factor(mat_levels_train_3, levels = c(0,1)),
  surv_3year_train_mat = surv_prob_train_3_mat
)
mat_surv_map_test_3 <- data.frame(
  mat01_cat = factor(mat_levels_test_3, levels = c(0,1)),
  surv_3year_test_mat = surv_prob_test_3_mat
)
mat_surv_map_train_5 <- data.frame(
  mat01_cat = factor(mat_levels_train_5, levels = c(0,1)),
  surv_5year_train_mat = surv_prob_train_5_mat
)
mat_surv_map_test_5 <- data.frame(
  mat01_cat = factor(mat_levels_test_5, levels = c(0,1)),
  surv_5year_test_mat = surv_prob_test_5_mat
)

cupi_surv_map_train_3 <- data.frame(
  CUPI01_cat = factor(cupi_levels_train_3, levels = c(0,1)),
  surv_3year_train_cupi = surv_prob_train_3_cupi
)
cupi_surv_map_test_3 <- data.frame(
  CUPI01_cat = factor(cupi_levels_test_3, levels = c(0,1)),
  surv_3year_test_cupi = surv_prob_test_3_cupi
)
cupi_surv_map_train_5 <- data.frame(
  CUPI01_cat = factor(cupi_levels_train_5, levels = c(0,1)),
  surv_5year_train_cupi = surv_prob_train_5_cupi
)
cupi_surv_map_test_5 <- data.frame(
  CUPI01_cat = factor(cupi_levels_test_5, levels = c(0,1)),
  surv_5year_test_cupi = surv_prob_test_5_cupi
)
...
train_data$surv_3year_mat <- mat_surv_map_train_3$surv_3year_train_mat[match(train_data$mat01_cat, mat_surv_map_train_3$mat01_cat)]
test_data$surv_3year_mat  <- mat_surv_map_test_3$surv_3year_test_mat[match(test_data$mat01_cat, mat_surv_map_test_3$mat01_cat)]
train_data$surv_5year_mat <- mat_surv_map_train_5$surv_5year_train_mat[match(train_data$mat01_cat, mat_surv_map_train_5$mat01_cat)]
test_data$surv_5year_mat  <- mat_surv_map_test_5$surv_5year_test_mat[match(test_data$mat01_cat, mat_surv_map_test_5$mat01_cat)]

train_data$surv_3year_cupi <- cupi_surv_map_train_3$surv_3year_train_cupi[match(train_data$CUPI01_cat, cupi_surv_map_train_3$CUPI01_cat)]
test_data$surv_3year_cupi  <- cupi_surv_map_test_3$surv_3year_test_cupi[match(test_data$CUPI01_cat, cupi_surv_map_test_3$CUPI01_cat)]
train_data$surv_5year_cupi <- cupi_surv_map_train_5$surv_5year_train_cupi[match(train_data$CUPI01_cat, cupi_surv_map_train_5$CUPI01_cat)]
test_data$surv_5year_cupi  <- cupi_surv_map_test_5$surv_5year_test_cupi[match(test_data$CUPI01_cat, cupi_surv_map_test_5$CUPI01_cat)]
...
train_data$pred_mat_km_train_3 <- 1 - train_data$surv_3year_mat
test_data$pred_mat_km_test_3  <- 1 - test_data$surv_3year_mat
train_data$pred_mat_km_train_5 <- 1 - train_data$surv_5year_mat
test_data$pred_mat_km_test_5  <- 1 - test_data$surv_5year_mat
train_data$pred_cupi_km_train_3 <- 1 - train_data$surv_3year_cupi
test_data$pred_cupi_km_test_3  <- 1 - test_data$surv_3year_cupi
train_data$pred_cupi_km_train_5 <- 1 - train_data$surv_5year_cupi
test_data$pred_cupi_km_test_5  <- 1 - test_data$surv_5year_cupi
...
dca_mat_3_train <- decision_curve(SUV ~ pred_mat_km_train_3, data = train_data, thresholds = seq(0, 0.5, 0.01))
dca_mat_3_test  <- decision_curve(SUV ~ pred_mat_km_test_3,  data = test_data,  thresholds = seq(0, 0.5, 0.01))
dca_mat_5_train <- decision_curve(SUV ~ pred_mat_km_train_5, data = train_data, thresholds = seq(0, 0.5, 0.01))
dca_mat_5_test  <- decision_curve(SUV ~ pred_mat_km_test_5,  data = test_data,  thresholds = seq(0, 0.5, 0.01))
dca_cupi_3_train <- decision_curve(SUV ~ pred_cupi_km_train_3, data = train_data, thresholds = seq(0, 0.5, 0.01))
dca_cupi_3_test  <- decision_curve(SUV ~ pred_cupi_km_test_3,  data = test_data,  thresholds = seq(0, 0.5, 0.01))
dca_cupi_5_train <- decision_curve(SUV ~ pred_cupi_km_train_5, data = train_data, thresholds = seq(0, 0.5, 0.01))
dca_cupi_5_test  <- decision_curve(SUV ~ pred_cupi_km_test_5,  data = test_data,  thresholds = seq(0, 0.5, 0.01))
...
plot_decision_curve(
  list(dca_3year, dca_cpab_3_train, dca_bclc_3_train, dca_mat_3_train, dca_cupi_3_train),
  curve.names = c("Nomogram", "CPC", "BCLC", "MAT", "CUPI", "Treat All", "Treat None"),
  cost.benefit.axis = FALSE,
  col = c("darkred", "darkorange", "skyblue", "purple", "darkgreen", "black", "grey"),
...
plot_decision_curve(
  list(dca_3year_test, dca_cpab_3_test, dca_bclc_3_test, dca_mat_3_test, dca_cupi_3_test),
  curve.names = c("Nomogram", "CPC", "BCLC", "MAT", "CUPI", "Treat All", "Treat None"),
  col = c("darkred", "darkorange", "skyblue", "purple", "darkgreen", "black", "grey"),
  cost.benefit.axis = FALSE,
  confidence.intervals = FALSE,
...
plot_decision_curve(
  list(dca_5year, dca_cpab_5_train, dca_bclc_5_train, dca_mat_5_train, dca_cupi_5_train),
  curve.names = c("Nomogram", "CPC", "BCLC", "MAT", "CUPI", "Treat All", "Treat None"),
  col = c("darkred", "darkorange", "skyblue", "purple", "darkgreen", "black", "grey"),
  cost.benefit.axis = FALSE,
  confidence.intervals = FALSE,
...
plot_decision_curve(
  list(dca_5year_test, dca_cpab_5_test, dca_bclc_5_test, dca_mat_5_test, dca_cupi_5_test),
  curve.names = c("Nomogram", "CPC", "BCLC", "MAT", "CUPI", "Treat All", "Treat None"),
  col = c("darkred", "darkorange", "skyblue", "purple", "darkgreen", "black", "grey"),
  cost.benefit.axis = FALSE,
  confidence.intervals = FALSE,
...
######
lp <- predict(cox_model_3, newdata = train_data, type = "lp")
pred_3y <- Survival(cox_model_3)(3, lp)
train_data$pred_3y <- pred_3y

km_res <- by(train_data, train_data$risk_group, function(d) {
  fit <- survfit(Surv(OS12, OS_event) ~ 1, data = d)
  s <- summary(fit, times = 3)
  data.frame(
    pred = mean(d$pred_3y),
    obs = s$surv,
    lower = s$lower,
...
cal_plot_df <- do.call(rbind, km_res)

plot(cal_plot_df$pred, cal_plot_df$obs,
     xlim = c(0, 1), ylim = c(0, 1),
...
arrows(cal_plot_df$pred,
       cal_plot_df$lower,
       cal_plot_df$pred,
       cal_plot_df$upper,
       angle = 90, code = 3, length = 0.05,col = "darkblue")

lines(cal_plot_df$pred, cal_plot_df$obs,col = "#1F77B4")
abline(0, 1, col = "grey")
legend("bottomright",
       legend = c("3-year OS", "Ideal line"),
       col = c("#1F77B4", "grey"),
       lty = c(1, 1),
       lwd = c(2, 1),
       pch = c(18, NA),
...
######
lp <- predict(cox_model_3, newdata = test_data, type = "lp")
pred_3y <- Survival(cox_model_3)(3, lp)

test_data$pred_3y <- pred_3y

km_res <- by(test_data, test_data$risk_group, function(d) {
  fit <- survfit(Surv(OS12, OS_event) ~ 1, data = d)
  s <- summary(fit, times = 3)
  data.frame(
    pred = mean(d$pred_3y),
...
arrows(cal_plot_df$pred, cal_plot_df$lower,
       cal_plot_df$pred, cal_plot_df$upper,
       angle = 90, code = 3, length = 0.05, col = "darkblue")
...
legend("bottomright",
       ...
       bty = "n")
#####
lp <- predict(cox_model_5, newdata = train_data, type = "lp")
pred_5y <- Survival(cox_model_5)(5, lp)

train_data$pred_5y <- pred_5y

km_res <- by(train_data, train_data$risk_group, function(d) {
  fit <- survfit(Surv(OS12, OS_event) ~ 1, data = d)
  s <- summary(fit, times = 5)
  data.frame(
    pred = mean(d$pred_5y),
    obs = s$surv,
    lower = s$lower,
...
legend("bottomright",
       ...
       bty = "n")
######
lp <- predict(cox_model_5, newdata = test_data, type = "lp")
pred_5y <- Survival(cox_model_5)(5, lp)
test_data$pred_5y <- pred_5y
km_res <- by(test_data, test_data$risk_group, function(d) {
  fit <- survfit(Surv(OS12, OS_event) ~ 1, data = d)
  s <- summary(fit, times = 5)
  data.frame(
...
train_compare_mat <- data.frame(
  time = train_data$OS12,
  status = train_data$OS_event,
  nomogram_3year = 1 - pred_3year,
  nomogram_5year = 1 - pred_5year,
  MAT_3year = train_data$pred_mat_km_train_3,
  MAT_5year = train_data$pred_mat_km_train_5
)
train_compare_cupi <- data.frame(
  time = train_data$OS12,
  status = train_data$OS_event,
  nomogram_3year = 1 - pred_3year,
  nomogram_5year = 1 - pred_5year,
  CUPI_3year = train_data$pred_cupi_km_train_3,
  CUPI_5year = train_data$pred_cupi_km_train_5
)
...
test_compare_mat <- data.frame(
  time = test_data$OS12,
  status = test_data$OS_event,
  nomogram_3year = 1 - pred_3year_test,
  nomogram_5year = 1 - pred_5year_test,
  MAT_3year = test_data$pred_mat_km_test_3,
  MAT_5year = test_data$pred_mat_km_test_5
)

test_compare_cupi <- data.frame(
  time = test_data$OS12,
  status = test_data$OS_event,
  nomogram_3year = 1 - pred_3year_test,
  nomogram_5year = 1 - pred_5year_test,
  CUPI_3year = test_data$pred_cupi_km_test_3,
  CUPI_5year = test_data$pred_cupi_km_test_5
)
# 计算训练集的NRI
...
train_nri_3year_mat <- nribin(
  event = train_compare_mat$status,
  p.std = train_compare_mat$MAT_3year,
  p.new = train_compare_mat$nomogram_3year,
  cut = c(0.2, 0.4),
  niter = 1000
)

train_nri_3year_cupi <- nribin(
  event = train_compare_cupi$status,
  p.std = train_compare_cupi$CUPI_3year,
  p.new = train_compare_cupi$nomogram_3year,
  cut = c(0.2, 0.4),
  niter = 1000
)
...
train_nri_5year_mat <- nribin(
  event = train_compare_mat$status,
  p.std = train_compare_mat$MAT_5year,
  p.new = train_compare_mat$nomogram_5year,
  cut = c(0.2, 0.4),
  niter = 1000
)
train_nri_5year_cupi <- nribin(
  event = train_compare_cupi$status,
  p.std = train_compare_cupi$CUPI_5year,
  p.new = train_compare_cupi$nomogram_5year,
  cut = c(0.2, 0.4),
  niter = 1000
)
...
test_nri_3year_mat <- nribin(
  event = test_compare_mat$status,
  p.std = test_compare_mat$MAT_3year,
  p.new = test_compare_mat$nomogram_3year,
  cut = c(0.2, 0.4),
  niter = 1000
)
test_nri_3year_cupi <- nribin(
  event = test_compare_cupi$status,
  p.std = test_compare_cupi$CUPI_3year,
  p.new = test_compare_cupi$nomogram_3year,
  cut = c(0.2, 0.4),
  niter = 1000
)
...
test_nri_5year_mat <- nribin(
  event = test_compare_mat$status,
  p.std = test_compare_mat$MAT_5year,
  p.new = test_compare_mat$nomogram_5year,
  cut = c(0.2, 0.4),
  niter = 1000
)
test_nri_5year_cupi <- nribin(
  event = test_compare_cupi$status,
  p.std = test_compare_cupi$CUPI_5year,
  p.new = test_compare_cupi$nomogram_5year,
  cut = c(0.2, 0.4),
  niter = 1000
)
...
train_idi_3year_mat <- calculate_idi_with_ci(
  time = train_compare_mat$time,
  status = train_compare_mat$status,
  old_pred = train_compare_mat$MAT_3year,
  new_pred = train_compare_mat$nomogram_3year,
  cutoff_time = 3
)

test_idi_3year_mat <- calculate_idi_with_ci(
  time = test_compare_mat$time,
  status = test_compare_mat$status,
  old_pred = test_compare_mat$MAT_3year,
  new_pred = test_compare_mat$nomogram_3year,
  cutoff_time = 3
)
train_idi_5year_mat <- calculate_idi_with_ci(
  time = train_compare_mat$time,
  status = train_compare_mat$status,
  old_pred = train_compare_mat$MAT_5year,
  new_pred = train_compare_mat$nomogram_5year,
  cutoff_time = 5
)

test_idi_5year_mat <- calculate_idi_with_ci(
  time = test_compare_mat$time,
  status = test_compare_mat$status,
  old_pred = test_compare_mat$MAT_5year,
  new_pred = test_compare_mat$nomogram_5year,
  cutoff_time = 5
)

train_idi_3year_cupi <- calculate_idi_with_ci(
  time = train_compare_cupi$time,
  status = train_compare_cupi$status,
  old_pred = train_compare_cupi$CUPI_3year,
  new_pred = train_compare_cupi$nomogram_3year,
  cutoff_time = 3
)

test_idi_3year_cupi <- calculate_idi_with_ci(
  time = test_compare_cupi$time,
  status = test_compare_cupi$status,
  old_pred = test_compare_cupi$CUPI_3year,
  new_pred = test_compare_cupi$nomogram_3year,
  cutoff_time = 3
)
train_idi_5year_cupi <- calculate_idi_with_ci(
  time = train_compare_cupi$time,
  status = train_compare_cupi$status,
  old_pred = train_compare_cupi$CUPI_5year,
  new_pred = train_compare_cupi$nomogram_5year,
  cutoff_time = 5
)

test_idi_5year_cupi <- calculate_idi_with_ci(
  time = test_compare_cupi$time,
  status = test_compare_cupi$status,
  old_pred = test_compare_cupi$CUPI_5year,
  new_pred = test_compare_cupi$nomogram_5year,
  cutoff_time = 5
)
...
# MAT（训练集）
cph_old_mat <- coxph(Surv(time, status) ~ MAT_3year, data = train_compare_mat)
cph_new_mat <- coxph(Surv(time, status) ~ nomogram_3year, data = train_compare_mat)

c_old_mat <- summary(cph_old_mat)$concordance
c_new_mat <- summary(cph_new_mat)$concordance

# 差异
c_diff_mat <- c_new_mat[1] - c_old_mat[1]
se_diff_mat <- sqrt(c_old_mat[2]^2 + c_new_mat[2]^2)

# CI + P值
ci_lower_mat <- c_diff_mat - 1.96 * se_diff_mat
ci_upper_mat <- c_diff_mat + 1.96 * se_diff_mat
z_score_mat <- c_diff_mat / se_diff_mat
p_value_mat <- 2 * pnorm(-abs(z_score_mat))

cat("Nomogram vs MAT (Train):\n")
cat(sprintf("C-index差异: %.4f (95%% CI: %.4f-%.4f)\n", c_diff_mat, ci_lower_mat, ci_upper_mat))
cat(sprintf("P值: %.4f\n\n", p_value_mat))

c_old_mat_val <- c_old_mat[1]
se_old_mat <- c_old_mat[2]

ci_lower_old_mat <- c_old_mat_val - 1.96 * se_old_mat
ci_upper_old_mat <- c_old_mat_val + 1.96 * se_old_mat

cat(sprintf("MAT (Train) C-index: %.4f (95%% CI: %.4f-%.4f)\n",
            c_old_mat_val, ci_lower_old_mat, ci_upper_old_mat))

# CUPI（训练集）
cph_old_cupi <- coxph(Surv(time, status) ~ CUPI_3year, data = train_compare_cupi)
cph_new_cupi <- coxph(Surv(time, status) ~ nomogram_3year, data = train_compare_cupi)

c_old_cupi <- summary(cph_old_cupi)$concordance
c_new_cupi <- summary(cph_new_cupi)$concordance

# 差异
c_diff_cupi <- c_new_cupi[1] - c_old_cupi[1]
se_diff_cupi <- sqrt(c_old_cupi[2]^2 + c_new_cupi[2]^2)

# CI + P值
ci_lower_cupi <- c_diff_cupi - 1.96 * se_diff_cupi
ci_upper_cupi <- c_diff_cupi + 1.96 * se_diff_cupi
z_score_cupi <- c_diff_cupi / se_diff_cupi
p_value_cupi <- 2 * pnorm(-abs(z_score_cupi))

cat("Nomogram vs CUPI (Train):\n")
cat(sprintf("C-index差异: %.4f (95%% CI: %.4f-%.4f)\n", c_diff_cupi, ci_lower_cupi, ci_upper_cupi))
cat(sprintf("P值: %.4f\n\n", p_value_cupi))

c_old_cupi_val <- c_old_cupi[1]
se_old_cupi <- c_old_cupi[2]

ci_lower_old_cupi <- c_old_cupi_val - 1.96 * se_old_cupi
ci_upper_old_cupi <- c_old_cupi_val + 1.96 * se_old_cupi

cat(sprintf("CUPI (Train) C-index: %.4f (95%% CI: %.4f-%.4f)\n",
            c_old_cupi_val, ci_lower_old_cupi, ci_upper_old_cupi))

# MAT（测试集）
cph_old_mat <- coxph(Surv(time, status) ~ MAT_3year, data = test_compare_mat)
cph_new_mat <- coxph(Surv(time, status) ~ nomogram_3year, data = test_compare_mat)
...
# CUPI（测试集）
cph_old_cupi <- coxph(Surv(time, status) ~ CUPI_3year, data = test_compare_cupi)
cph_new_cupi <- coxph(Surv(time, status) ~ nomogram_3year, data = test_compare_cupi)
...
c_old_mat_test_val <- c_old_mat[1]
se_old_mat_test <- c_old_mat[2]

ci_lower_old_mat_test <- c_old_mat_test_val - 1.96 * se_old_mat_test
ci_upper_old_mat_test <- c_old_mat_test_val + 1.96 * se_old_mat_test

cat(sprintf("MAT (Test) C-index: %.4f (95%% CI: %.4f-%.4f)\n",
            c_old_mat_test_val, ci_lower_old_mat_test, ci_upper_old_mat_test))

c_old_cupi_test_val <- c_old_cupi[1]
se_old_cupi_test <- c_old_cupi[2]

ci_lower_old_cupi_test <- c_old_cupi_test_val - 1.96 * se_old_cupi_test
ci_upper_old_cupi_test <- c_old_cupi_test_val + 1.96 * se_old_cupi_test

cat(sprintf("CUPI (Test) C-index: %.4f (95%% CI: %.4f-%.4f)\n",
            c_old_cupi_test_val, ci_lower_old_cupi_test, ci_upper_old_cupi_test))